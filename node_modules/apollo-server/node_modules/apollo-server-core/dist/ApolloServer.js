...
var apollo_engine_reporting_1 = require("apollo-engine-reporting");
...
var ApolloServerBase = (function () {
    function ApolloServerBase(config) {
        ...
        var context = config.context, resolvers = config.resolvers, schema = config.schema, schemaDirectives = config.schemaDirectives, typeDefs = config.typeDefs, introspection = config.introspection, mocks = config.mocks, extensions = config.extensions, engine = config.engine, requestOptions = __rest(config, ["context", "resolvers", "schema", "schemaDirectives", "typeDefs", "introspection", "mocks", "extensions", "engine"]);
        ...
        if (typeof typeDefs === 'string' ||
            (Array.isArray(typeDefs) && typeDefs.find(function (d) { return typeof d === 'string'; }))) {
            var startSchema = (typeof typeDefs === 'string' &&
                typeDefs.substring(0, 200)) ||
                (Array.isArray(typeDefs) &&
                    typeDefs.find(function (d) { return typeof d === 'string'; }).substring(0, 200));
            throw new Error("typeDefs must be tagged with the gql exported from apollo-server:\n\nconst { gql } = require('apollo-server');\n\nconst typeDefs = gql`" + startSchema + "`\n");
        }
        var enhancedTypeDefs = Array.isArray(typeDefs)
        ...
        if (engine || (engine !== false && process.env.ENGINE_API_KEY)) {
            ...
            this.extensions.push(function () { return _this.engineReportingAgent.newExtension(); });
        }
        if (extensions) {
    ...
    ApolloServerBase.prototype.enhanceSchema = function (schema) {
    ...
    ApolloServerBase.prototype.createEngine = function (_a) {
        ...
        if (this.engineProxy || engineInRequestPath) {
            this.extensions.push(function () { return new apollo_tracing_1.TracingExtension(); });
            this.extensions.push(function () { return new apollo_cache_control_1.CacheControlExtension(); });
        }
    };
    ApolloServerBase.prototype.graphQLServerOptionsForRequest = function (request) {
        return __awaiter(this, void 0, void 0, function () {
            var context, _a, error_1;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                    ...
                    case 5:
                        ...
                        return [3, 6];
                    case 6: return [2, __assign({ schema: this.schema, extensions: this.extensions, context: context, logFunction: this.requestOptions.logFunction, fieldResolver: this.requestOptions.fieldResolver }, this.requestOptions)];
                }
            });
        });
    };
