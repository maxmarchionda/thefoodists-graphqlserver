...
var gql = String.raw;
var fileUploadMiddleware = function (uploadsConfig, server) { return function (req, res, next) {
...
exports.registerServer = function (_a) {
    var app = _a.app, server = _a.server, path = _a.path, cors = _a.cors, bodyParserConfig = _a.bodyParserConfig, disableHealthCheck = _a.disableHealthCheck, onHealthCheck = _a.onHealthCheck, uploads = _a.uploads;
    return __awaiter(_this, void 0, void 0, function () {
        var uploadsMiddleware;
        return __generator(this, function (_b) {
            if (!path)
                path = '/graphql';
            if (!disableHealthCheck) {
                app.use('/.well-known/apollo/server-health', function (req, res) {
            ...
            server.use({
                path: path,
                getHttp: function () { return http_1.createServer(app); },
            });
            app.use(path, corsMiddleware(cors), body_parser_1.json(bodyParserConfig), uploadsMiddleware ? uploadsMiddleware : function (_req, _res, next) { return next(); }, function (req, res, next) {
                if (!server.disableTools && req.method === 'GET') {
                    var accept = accepts(req);
                    var types = accept.types();
                    var prefersHTML = types.find(function (x) { return x === 'text/html' || x === 'application/json'; }) === 'text/html';
                    if (prefersHTML) {
